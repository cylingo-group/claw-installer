# op-dispatch-protocol Specification

## Purpose

Define the single unified path by which the Rust GUI invokes bash business logic across both Windows (via `bootstrap.ps1`) and macOS/Linux (via `claw-op.sh`). Each named operation routes through a per-OS glue layer that sources `common.sh` (guaranteeing fnm-managed Node, pnpm, and other installer-managed tools are on PATH), then execs a script under `shell/agents/<agent>/<op>.sh`. The protocol fixes input conventions (stdin for secrets/JSON, `INSTALLER_OP_*` env vars for scalar params, argv `--op`/`--agent` for routing) so all op scripts share one shape — eliminating the ad-hoc inline-shell-in-Rust patterns that previously caused PATH-related failures like `exec: node: not found`.
