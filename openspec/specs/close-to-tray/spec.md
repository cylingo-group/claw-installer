# close-to-tray Specification

## Purpose

Route window-close gestures to a hide-to-tray behavior rather than process exit, and provide a persistent system tray icon on both macOS (menu bar) and Windows (notification area) with a contextual menu (Open, Start All Agents, Stop All Agents, Quit). This keeps installed agents reachable in the background — a user who dismisses the window can re-open it or trigger start/stop from the tray without restarting the process.
