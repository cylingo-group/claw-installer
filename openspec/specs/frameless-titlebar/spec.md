# frameless-titlebar Specification

## Purpose

Replace OS-native window chrome with a custom React titlebar so the app's visual identity is consistent across macOS and Windows. macOS gets traffic-light style buttons (red close, yellow minimize, dimmed green); Windows gets minimize and close icons matched to Windows 11 Snap Layout sizing. The titlebar is a draggable region; maximize is explicitly absent. Buttons remain keyboard-focusable for accessibility.
