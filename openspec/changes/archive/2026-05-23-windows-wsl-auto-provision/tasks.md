...
**T0.1 — `InstallerEvent::RebootRequired` in Rust** [x]
...
**T0.2 — `InstallerEvent` union in TypeScript** [x]
...
**T1.1 — Fix `Copy-RepoIntoWsl` to copy `agents/` directory** [x]
...
**T2.1 — Add `-Agent` and `-Service` to `param()` block** [x]
...
**T2.2 — Extend `Invoke-BashInstaller` to support `-Agent`** [x]
...
**T2.3 — Add `Invoke-BashService` function** [x]
...
**T2.4 — `CLAW_*` env-var forward allow-list** [x]
...
**T3.1 — Add `Assert-Elevated` function** [x]
...
**T3.2 — Rename `Test-WslAvailable` → `Test-WslAvailable_Quick`** [x]
...
**T3.3 — Add `Install-WslFeatures` and `Ensure-WslInstalled` functions** [x]
...
**T3.4 — Update `Ensure-Distro` to auto-install** [x]
...
**T4.1 — `build_command`: forward `-Agent` on Windows when single-agent** [x]
...
**T4.2 — `build_service_command`: add Windows branch** [x]
...
**T4.3 — `run_uninstaller`: set `CLAW_UNINSTALL_AGENT` on Windows** [x]
...
**T4.4 — `run_event_loop`: emit `RebootRequired` on exit code 2** [x]
...
**T4.5 — Add `system_reboot` Tauri command** [x]
...
**T4.6 — Capability file: add `shell:allow-execute` for `shutdown.exe`** [x]
...
**T5.1 — `handleInstallerEvent`: handle `RebootRequired`** [x]
...
**T5.4 — Mount `RebootModal` in `App.tsx`** [x]
...
**T6.1 — DryRun matrix on Windows** [x]
...
**T6.2 — `Copy-RepoIntoWsl` layout test** [x]
...
**T6.3 — Rust compile check on all targets** [x]
...
**T6.4 — GUI RebootModal display test (stub mode)** [x]
...
**T6.5 — End-to-end: service action on Windows** [x]
