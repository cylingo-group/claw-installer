...
step_python() {
  display "@@step:python:Installing Python 3.11…"
  command -v uv >/dev/null 2>&1 || die_step "Install Python" "uv not on PATH — run steps/uv.sh first" 1
  ...
  if uv python find "$version" >/dev/null 2>&1; then
    ...
    display "Python $version is ready; skipping install"
