...
step_hermes_node() {
  display "@@step:hermes-node:Configuring Hermes-specific Node runtime…"
  ...
  if [[ -x "$node_bin" ]]; then
    ...
    display "Hermes-specific Node already in place; skipping config"
  ...
  if command -v node >/dev/null 2>&1; then
    if node -e 'process.exit(process.versions.node.split(".").map(Number)[0] >= 22 ? 0 : 1)' 2>/dev/null; then
      ...
      display "Node $(node --version) on PATH satisfies Hermes' requirement; skipping dedicated Node"
      manifest_record hermes_node_symlink "$node_dir" preexisting "system Node $(node --version) satisfies upstream check"
      return
    fi
  fi

  command -v fnm >/dev/null 2>&1 || die_step "Configure Hermes Node" "fnm not on PATH — run steps/fnm.sh first" 1
  ...
  [[ -x "$node22_path" ]] || die_step "Configure Hermes Node" "Could not resolve fnm Node v22 binary (got: '$node22_path')" 1
  ...
  display "✓ Hermes-specific Node v22 configured"
